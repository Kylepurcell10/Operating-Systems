...
int maxInts = SYSTEM_MAX_MEMORY_SIZE / sizeof(int);     //total of 128 memory blocks
    memoryStack = (int*) calloc(maxInts , sizeof(int)); //dynamically assigns memory
        ...
        if(memoryThreads.fit_Type == 1){                //print functions to return fit type
...
void* memory_malloc(void* threadNum){
    while(1){  
        ...
        Start:
                ...
                if(memoryThreads.fit_Type == 1){                                //FIRST FIT
                    for(int i = 0 ; i < 128 ; i++){
        SEARCHING:      if(memoryStack[i] == 0){                             //Scan the whole memory and see if there is enough space
...
/*If you find a pointer that is greater than the pointer that is currently being removed, store it and then pass it to the next if statement. The if statement asks if the thread pointer that is bigger than the thread pointer being removed, is the smallest of the pointers, this makes it the one that needs to be moved first. This statement is essentially looking for the smallest pointer bigger than the pointer being removed.*/

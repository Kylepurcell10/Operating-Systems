...
void *produce(); // initialize the functions for the producer and
consumer
...
int main(int argc , char *argv[]){
 ...
 pthread_t producer; 
 pthread_t consumer[count]; // varying size array of consumers to allow
for user input
 int *threadNum[count];
 sem_init(&empty , 1 , 1); // initialize semophores and their starting
values
 sem_init(&full , 1 , 0);
 sem_init(&lock , 1 , 1);
...
while(1){
 ...
 for(int i=0 ; i < 5 ; i++){ //scan through the buffer and find a
nonzero number
 if(BUFFER[i] > 0){ //after you find one, obtain the value and
set it to zero
...
&& BUFFER[4] == 0){ // condition for empty to be true
